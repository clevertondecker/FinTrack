...
public class InvoiceController {
     ...
     * @param id the invoice ID.
     * @param userDetails the authenticated user details.
     * @return a response with detailed invoice information.
     */
    @GetMapping("/{id}/debug")
    public ResponseEntity<Map<String, Object>> debugInvoice(
            @PathVariable Long id,
            @AuthenticationPrincipal UserDetails userDetails) {
        ...
        // Debug information
        List<Map<String, Object>> itemDebug = new ArrayList<>();
        for (InvoiceItem item : invoice.getItems()) {
            itemDebug.add(Map.of(
                "id", item.getId(),
        ...
        return ResponseEntity.ok(Map.of(
            ...
            "invoice", Map.of(
                ...
                "dueDate", invoice.getDueDate(),
                "totalAmount", invoice.getTotalAmount(),
                "itemsCount", invoice.getItems().size(),
                "items", itemDebug,
                "status", invoice.getStatus().name(),
                "createdAt", invoice.getCreatedAt(),
                "updatedAt", invoice.getUpdatedAt()
